/*  mslltd.c    merge sort linked list top down */
#include <stdio.h>
#include <stdlib.h>
...
NODE *MergeSortR(NODE * pList, size_t count)
{
NODE * pNode = pList;
NODE * pNext;
size_t end;
size_t cnt;
    ...
    pNext = pNode->next;
    pNode->next = NULL;
    pList = MergeSortR(pList, ++cnt);
    pNext = MergeSortR(pNext, count-cnt);
    return Merge(pList, pNext);
}
