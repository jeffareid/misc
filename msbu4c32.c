...
void MergeSort(uint32_t a[], size_t n, CMPFUNC cmp)
{
...
uint32_t *p2r;                              /* ptr to current element run 2 */
uint32_t *p2e;                              /* ptr to end             run 2 */
uint32_t *p3r;                              /* ptr to current element run 3 */
uint32_t *p3e;                              /* ptr to end             run 3 */
uint32_t *pax;                              /* ptr to a[] or b[] */
uint32_t *pbx;                              /* ptr to b[] or a[] */
uint32_t * b;                               /* second array */
size_t rsz;                                 /* run size  */
...
uint32_t rnd32()                                /* random 32 bit integer */
{
...
int main()
{
uint32_t * a = malloc(COUNT * sizeof(uint32_t));
size_t i;
    for(i = 0; i < COUNT; i++)
        a[i] = rnd32();
#if QP
    ...
    Sleep(500);                             /* wait for it to settle */
    QueryPerformanceCounter(&liStartTime);
#else
    ctTimeStart = clock();
#endif
    MergeSort(a, COUNT, cmp_uint32);
#if QP
