/*----------------------------------------------------------------------*/
/*      eccr16v.c       ecc rs GF(2^4) + Vandermonde                    */
/*                                                                      */
/*      Jeff Reid       2024OCT25 16:30                                 */
/*----------------------------------------------------------------------*/
...
/* powers of alpha from 0 to n-2 */
#define FOURIER 0

typedef unsigned char BYTE;

#if FOURIER
/* input values are powers of alpha */
#define a(x) (GFPow(bAlpha, (x)))
#define MAXDAT 15
#define MAXPAR 12
#else
/* inputs are values 0,1,... */
#define a(x) ((BYTE)(x))
#define MAXDAT 16
#define MAXPAR 12
#endif
...
int r, c;
    ...
    for(r = 0; r < K; r++)
        for(c = 0; c < N; c++)
#if FOURIER
            mV.data[r*N+c] = GFPow(GFPow(2,c),r);
#else
            mV.data[r*N+c] = GFPow(c,r);
#endif
#if DISPLAYV
    printf("Vandermonde\n");
    ShowMatrix(&mV);
...
BYTE t;
    ...
    for(j = 0; j < N; j++){             /* check result | encode */
        t = vPoly.data[0];
        for(i = 1; i < K; i++)
            t = GFAdd(GFMpy(t, a(j)), vPoly.data[i]);
...
loop:
    ...
    for(j = 0; j < mM.nrows; j++){
        ...
        for(i = 0; i < e; i++)
            prowj[i] = GFMpy(vData.data[j],GFPow(a(j),i));
        for(; i < mM.ncols-1; i++)
            prowj[i] = GFSub(0, GFPow(a(j), (i-e)));
        prowj[i] = GFSub(0, GFMpy(vData.data[j], GFPow(a(j),e)));
    }
#if DISPLAYM
    printf("\n"); ShowMatrix(&mM);
#endif
...
err0:
    ...
    for(j = 0; j < vE.size; j++){
        ...
        for(i = 1; i < vP.size; i++)
            t = GFAdd(GFMpy(t, a(f[j])), vP.data[i]);
        vData.data[f[j]] = t;
    }
