...
typedef struct{                         /* euclid structure */
    ...
    BYTE data[258];                     /* left and right side data */
}EUCLID;
...
static BYTE     abInv[256];
static int      f[256];                 /* fix indexes */

static BYTE     abId[512];              /* array for MatrixInv */
...
static VECTOR   pP[256];                /* polynomials */
static VECTOR   pL[256];                 /* L[] polynomials */
static VECTOR   pS[256];                 /* S[] syndrome polynomials */
#endif
...
/*      select Galios Field */
    while(1){
        ...
        for(j = 0; j < 3; j++){
            printf("%02d:", u0/2);
            for(i = 0; i < 10; i++){
                printf(" %03x %02x", aiGA[u0], aiGA[u0+1]);
                u0 += 2;}
            printf("\n");}
...
/*      get user command */
    ...
    switch(abUser[2] & 0x5f){
      ...
      case 'C':                         /* change data */
        while(1){
            printf("Enter offset in hex (0-%x): ", vData.size-1);
            if(!Conrs())break;
            sscanf(&abUser[2], "%d", &u0);
