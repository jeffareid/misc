/*----------------------------------------------------------------------*/
/*      eccr17z.c       ecc rs GF(17)                                   */
/*                                                                      */
/*      Jeff Reid       2025AUG16 17:45                                 */
/*----------------------------------------------------------------------*/
...
typedef struct{                         /* euclid structure */
    ...
    WORD data[MAXPAR+2];                /* left and right side data */
}EUCLID;
...
/*----------------------------------------------------------------------*/
static WORD     wGF = 7;
static WORD     wAlpha = ALPHA;

static WORD     awExp[GF*2];
static WORD     awLog[GF];
static WORD     awInv[GF];
static WORD     awNeg[GF];

static int      f[GF];                  /* fix indexes */

static WORD     awId[GF*2];             /* array for MatrixInv */
...
static BYTE     ab17[18] = "0123456789abcdefg";
...
static WORD     Poly2Val(VECTOR *, WORD);
static WORD     GFAdd(WORD, WORD);
static WORD     GFSub(WORD, WORD);
static WORD     GFMpy(WORD, WORD);
static WORD     GFDiv(WORD, WORD);
static WORD     GFPow(WORD, WORD);
...
loop:
    ...
    while(vQ.size != 0 && vQ.data[0] == 0){   /* remove leading zeroes */
        vQ.size--;
        memcpy(vQ.data, vQ.data+1, vQ.size*sizeof(WORD));
    }
#if DISPLAYD
    printf("vQ:\n"); ShowVector(&vQ);
    printf("vE:\n"); ShowVector(&vE);
...
/*      do Euclid algorithm */
    ...
    while(1){                           /* while deg of pE1[] >= (N+K)/2) */
        ...
        while((pE1->data[0] == 0) &&    /* shift dvsr left until msb!=0 */
            ...
            pE1->indx--;
            memcpy(&pE1->data[0], &pE1->data[1], (pE1->size-1)*sizeof(WORD));
        ...
        while(1){                       /* while more divide sub-steps */
            ...
            if(pE0->indx == pE1->indx)  /*   if divide done, break */
                break;
            pE0->indx--;                /*   shift dvnd */
            memcpy(&pE0->data[0], &pE0->data[1], (pE0->size-1)*sizeof(WORD));
...
/*                                      ** this changes for GF(2^n) */
    pD.size = pE.size-1;                /* pD = derivative of pE */
    for(i = 0; i < pD.size; i++)
        pD.data[i] = GFMpy((WORD)(pD.size-i), pE.data[i]);
#if DISPLAYS
    printf("pD: ");
    ShowVector(&pD);
#endif
    ...
    for(i = 0; i < vV.size; i++){
        t = GFMpy(GFSub(0, vX.data[i]), Poly2Val(&pO, awInv[vX.data[i]]));
        t = GFDiv(t, GFMpy(vY.data[i], Poly2Val(&pD, awInv[vX.data[i]])));
        vV.data[i] = t;
    }
    printf("vV: ");
    ShowVector(&vV);
...
WORD *pbs0, *pbs1;                      /* ptr to src */
    ...
    pMDst->ncols = dcols;
    memset(pMDst->data, 0, drows*dcols*sizeof(WORD));
...
{
int     i, j;
WORD    *p0;                            /* generic pointers */
WORD    *p1;
WORD    *p2;
int     iNCol;                          /* # columns */
int     iNCol2;                         /* 2 * # columns */
int     iNCol21;                        /* 1+2*# columns */
WORD    bMod;                           /* row modifier */
WORD    bTemp;

    memset(awId, 0, sizeof(awId));      /* set up awId */
    awId[K] =  1;
...
/*      left side is copy of pMSrc, right side is Identity Matrix */
    ...
    p1  = pMDst->data;
    for(i = 0; i < iNCol; i++){
        memcpy(p1,  p0, iNCol*sizeof(WORD)); /* copy a row of data */
        p0  += iNCol;
        p1  += iNCol;
        memcpy(p1,  &awId[K-i], iNCol*sizeof(WORD)); /* add ID  part */
        p1  += iNCol;}
...
int i, j;
    ...
    pPDst->data[0] = 1;
    memset(&pPDst->data[1], 0, pVSrc->size*sizeof(WORD));
...
/*----------------------------------------------------------------------*/
static WORD Poly2Val(VECTOR *pPoly, WORD val)
{
int s = pPoly->size-1;
int i;
WORD v = 0;
...
/*      GFAdd(num0, num1)                                               */
/*----------------------------------------------------------------------*/
static WORD GFAdd(WORD num0, WORD num1)
{
WORD sum;
    sum = (num0+num1)%GF;
    return(sum);
...
/*      GFSub(num0, num1)                                               */
/*----------------------------------------------------------------------*/
static WORD GFSub(WORD num0, WORD num1)
{
WORD dif;
    dif = (GF+num0-num1)%GF;
    return(dif);
...
/*      GFMpy(mpnd, mplr)                                               */
/*----------------------------------------------------------------------*/
static WORD GFMpy(WORD mpnd, WORD mplr)
{
    if((mpnd == 0) || (mplr == 0))
        return(0);
    return(awExp[awLog[mpnd] + awLog[mplr]]);
}

/*----------------------------------------------------------------------*/
/*      GFDiv(dvnd, dvsr)                                               */
/*----------------------------------------------------------------------*/
static WORD GFDiv(WORD dvnd, WORD dvsr)
{
    if(dvsr == 0){
    ...
    if(dvnd == 0){
        return(0);}
    return(awExp[(GF-1) + awLog[dvnd] - awLog[dvsr]]);
}

/*----------------------------------------------------------------------*/
/*      GFPow(mpnd, expn)                                               */
/*----------------------------------------------------------------------*/
static WORD GFPow(WORD mpnd, WORD expn)
{
WORD powr = 1;
    while(expn--)
...
int k;
    ...
    for(i = 0; i < GF-1; i++){
        ...
        j %= GF;}
    awLog[0] = -1;
    memcpy(&awExp[GF-1], &awExp[0], (GF-1)*sizeof(WORD));

    awInv[0] = 0xff;                    /* init awInv[] */
    for(i = 1; i < GF; i++)
        awInv[i] = GFDiv(1,i);

    awNeg[0] = 0;                       /* init awNeg[] */
    for(i = 1; i < GF; i++)
        awNeg[i] = GF-i;
