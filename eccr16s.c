/*----------------------------------------------------------------------*/
/*      eccr16s.c       ecc rs GF(2^4) + syndromes                      */
/*                                                                      */
/*      Jeff Reid       2025AUG12 14:30                                 */
/*----------------------------------------------------------------------*/
...
static VECTOR   pS[16];                 /* syndrome polynomial */
#endif
...
int     iSum;                           /* current sum */
    ...
    for(j = 0; j < vData.size;  j++){
        ...
        iLcr = GFMpy(iLcr, bAlpha);}    /* set up next higher Alpha */
...
BYTE t;
    ...
    vA.size = 15;                       /* init vA (any 15 non-zeroes) */
    memcpy(vA.data, "\x7\xf\x3\x1\x5\xd\x9\xa\x2\x8\xb\x4\xc\x6\xe", 15);
